...
namespace Voy.AvatarHelpers
{
    public class TextureVRAM : EditorWindow
    {
        ...
        [MenuItem("GameObject/Voy/Avatar/VRAM", true, 0)]
        static bool CanShowFromSelection() => Selection.activeGameObject != null;

        [MenuItem("GameObject/Voy/Avatar/VRAM", false, 0)]
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField($"<size=20><color=magenta>CVR Avatar VRAM Calculator</color></size> v{AvatarEvaluator.VERSION}", new GUIStyle(EditorStyles.label) { richText = true, alignment = TextAnchor.MiddleCenter });
            EditorGUILayout.LabelField(AvatarEvaluator.VERSIONBASED, EditorStyles.centeredGreyMiniLabel);
            if (GUILayout.Button("Based on work by Thry, Click here to follow them on twitter", EditorStyles.centeredGreyMiniLabel))
            ...
            if (_avatar != null)
            {
                ...
                EditorGUILayout.BeginHorizontal(GUI.skin.box);
                    EditorGUILayout.LabelField("Combined (all): ", AvatarEvaluator.ToMebiByteString(_sizeAll), GUILayout.Width(250));
                    EditorGUILayout.LabelField("Combined (only active): ", AvatarEvaluator.ToMebiByteString(_sizeActive), GUILayout.Width(250));
                    GUILayout.FlexibleSpace();
                EditorGUILayout.EndHorizontal();
